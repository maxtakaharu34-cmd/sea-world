...
const SEA_CHARS=[
  {id:'diver',  name:'ダイブr', color:'#b3e5fc',accent:'#f57f17',hat:'#ffd54f'},
  {id:'mermaid',name:'マーメイド', color:'#80cbc4',accent:'#e040fb',hat:'#f8bbd9'},
  {id:'crab',   name:'カニ',  color:'#ef9a9a',accent:'#ffd180',hat:'#ff5252'},
];
const SEA_レベルS:LevelData[]=[
  {id:1,name:'珊瑚の海岸',bgColors:['#006064','#00838f'] as [string,string],width:3200,spawnX:80,spawnY:300,goal:{x:3100,y:200},
  ...
  {id:2,name:'海賊の入江',bgColors:['#004d40','#00695c'] as [string,string],width:3600,spawnX:80,spawnY:350,goal:{x:3500,y:160},
  ...
  {id:3,name:'深海の闇',bgColors:['#1a0030','#003040'] as [string,string],width:4000,spawnX:80,spawnY:360,goal:{x:3900,y:150},
...
function draw(ctx:CanvasRenderingContext2D,players:Player[],platforms:Platform[],coins:Coin[],powerups:Powerup[],enemies:Enemy[],camX:number,lvl:LevelData,level:number,total:number){
  ...
  ctx.restore();
  players.forEach((p,i)=>{const ox=i===0?10:CANVAS_W-180;ctx.fillStyle='rgba(0,40,40,0.6)';rr2(ctx,ox,8,165,48,8);ctx.fill();ctx.fillStyle='#80cbc4';ctx.font='bold 13px monospace';ctx.fillText(`P${i+1} ${SEA_CHARS.find(c=>c.id===p.charId)?.name||'ダイブr'}`,ox+10,26);ctx.fillStyle='#4dd0e1';ctx.font='bold 12px monospace';ctx.fillText(`🐚 ${p.coins}`,ox+10,43);ctx.fillStyle='#fff';ctx.fillText(`${p.score}pt`,ox+65,43);});
  ctx.fillStyle='rgba(0,40,40,0.6)';rr2(ctx,CANVAS_W/2-60,8,120,28,8);ctx.fill();ctx.fillStyle='#80cbc4';ctx.font='bold 13px monospace';ctx.textAlign='center';ctx.fillText(`レベル ${level} / ${total}`,CANVAS_W/2,26);ctx.textAlign='left';
}
...
export default function Index(){
  ...
  const loadLevel=useCallback((idx:number,char:string)=>{const data=SEA_レベルS[idx];levelDataRef.current=data;const{platforms,coins,powerups,enemies}=buildLevel(data);platformsRef.current=platforms;coinsRef.current=coins;powerupsRef.current=powerups;enemiesRef.current=enemies;camXRef.current=0;playersRef.current=[new Player(data.spawnX,data.spawnY,char as CharId,0)];},[]);
  ...
  useEffect(()=>{const down=(e:KeyboardEvent)=>{keysRef.current.add(e.key);if(['ArrowUp',' '].includes(e.key)){e.preventDefault();const p=playersRef.current[0];if(p&&(p.onGround||p.jumpsLeft>0)){p.jump();sfx(sfxジャンプ);}}if(e.key==='Enter'){if(screenRef.current==='level_complete'){const n=levelRef.current+1;if(n<=SEA_レベルS.length)startGame(n);else setScreen('menu');}if(screenRef.current==='gameover')startGame(levelRef.current);}};const up=(e:KeyboardEvent)=>keysRef.current.delete(e.key);window.addEventListener('keydown',down);window.addEventListener('keyup',up);return()=>{window.removeEventListener('keydown',down);window.removeEventListener('keyup',up);};},[sfx,startGame]);
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      if(s!=='playing'){draw(ctx,playersRef.current,platformsRef.current,coinsRef.current,powerupsRef.current,enemiesRef.current,camXRef.current,lvl,levelRef.current,SEA_レベルS.length);if(s==='level_complete'||s==='gameover'){ctx.fillStyle='rgba(0,0,0,0.72)';ctx.fillRect(0,0,CANVAS_W,CANVAS_H);ctx.textAlign='center';if(s==='level_complete'){ctx.fillStyle='#80cbc4';ctx.font='bold 36px monospace';ctx.fillText('🐚 CLEAR!',CANVAS_W/2,CANVAS_H/2-50);ctx.fillStyle='#fff';ctx.font='16px monospace';ctx.fillText(`スコア: ${playersRef.current[0]?.score||0}`,CANVAS_W/2,CANVAS_H/2);ctx.fillStyle='#4dd0e1';ctx.font='14px monospace';ctx.fillText('ENTERで次のレベル',CANVAS_W/2,CANVAS_H/2+40);}else{ctx.fillStyle='#ef5350';ctx.font='bold 34px monospace';ctx.fillText('ゲームオーバー',CANVAS_W/2,CANVAS_H/2-40);ctx.fillStyle='#fff';ctx.font='16px monospace';ctx.fillText(`スコア: ${playersRef.current[0]?.score||0}`,CANVAS_W/2,CANVAS_H/2+5);ctx.fillStyle='#80cbc4';ctx.font='14px monospace';ctx.fillText('ENTERで再挑戦',CANVAS_W/2,CANVAS_H/2+45);}ctx.textAlign='left';}return;}
      ...
      draw(ctx,players,platforms,coinsRef.current,powerupsRef.current,enemiesRef.current,camXRef.current,lvl,levelRef.current,SEA_レベルS.length);
    };
  ...
  return(
    <div className="min-h-screen bg-gradient-to-b from-[#001a1a] via-[#002b2b] to-[#001a1a] flex flex-col items-center justify-center p-4">
      <div className="text-center mb-4">
        <h1 className="text-4xl md:text-5xl font-black tracking-tight bg-gradient-to-r from-teal-300 via-cyan-300 to-blue-400 bg-clip-text text-transparent">🌊 海の世界</h1>
        <p className="text-teal-400/60 text-sm mt-1">貝を集めて · 海の生き物を避けて · 宝物を目指せ!</p>
      </div>
      <div className="flex flex-col items-center gap-4 select-none">
        ...
        {screen==='menu'&&(<div className="flex flex-col items-center gap-4 w-full max-w-sm"><div className="flex justify-center gap-2">{SEA_CHARS.map(c=>(<button key={c.id} onClick={()=>setP1Char(c.id)} className={`px-3 py-1 rounded-lg text-xs font-bold border-2 transition-all ${p1Char===c.id?'border-yellow-400 scale-110':'border-gray-600'}`} style={{background:c.accent+'22',color:c.accent}}>{c.name}</button>))}</div><div className="flex gap-3"><button onClick={()=>startGame(1)} className="px-8 py-3 bg-gradient-to-r from-teal-700 to-cyan-700 text-white rounded-xl text-lg font-bold hover:scale-105 transition-transform shadow-lg">🌊 ダイブ！</button><button onClick={()=>setSoundOn(s=>!s)} className="px-4 py-3 bg-gray-700 text-white rounded-xl text-sm hover:bg-gray-600">{soundOn?'🔊':'🔇'}</button></div><p className="text-gray-500 text-xs">←→ 移動 · ↑ ジャンプ · Collect 🐚 shells · Dodge 🦀🪼🦞</p></div>)}
        {screen==='playing'&&(<div className="flex flex-col gap-2 md:hidden w-full max-w-xs"><div className="flex justify-center"><button onTouchStart={(e)=>{e.preventDefault();playersRef.current[0]?.jump();sfx(sfxジャンプ);}} className="w-14 h-14 bg-teal-800 rounded-xl text-2xl font-bold text-white flex items-center justify-center">↑</button></div><div className="flex justify-center gap-3"><button onTouchStart={(e)=>{e.preventDefault();keysRef.current.add('ArrowLeft');}} onTouchEnd={(e)=>{e.preventDefault();keysRef.current.delete('ArrowLeft');}} className="w-14 h-14 bg-teal-800 rounded-xl text-2xl font-bold text-white flex items-center justify-center">←</button><button onTouchStart={(e)=>{e.preventDefault();keysRef.current.add('ArrowRight');}} onTouchEnd={(e)=>{e.preventDefault();keysRef.current.delete('ArrowRight');}} className="w-14 h-14 bg-teal-800 rounded-xl text-2xl font-bold text-white flex items-center justify-center">→</button></div></div>)}
        {screen==='level_complete'&&<button onClick={()=>{const n=currentLevel+1;if(n<=SEA_レベルS.length)startGame(n);else setScreen('menu');}} className="px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 text-white rounded-xl font-bold hover:scale-105">{currentLevel<SEA_レベルS.length?'次のレベル →':'🏠 Menu'}</button>}
        {screen==='gameover'&&<div className="flex gap-3"><button onClick={()=>startGame(currentLevel)} className="px-6 py-3 bg-gradient-to-r from-teal-700 to-cyan-700 text-white rounded-xl font-bold hover:scale-105">もう一度</button><button onClick={()=>setScreen('menu')} className="px-6 py-3 bg-gray-700 text-white rounded-xl font-bold hover:bg-gray-600">メニュー</button></div>}
      </div>
    </div>
  );
}
